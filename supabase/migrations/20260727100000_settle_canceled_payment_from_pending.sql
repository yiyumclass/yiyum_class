-- 승인 직후 이용권 발급이 끊긴 주문(pending/failed)에 CANCELED 웹훅이 도착하면
-- 기존 함수는 'paid order required'로 거절해 취소 사실이 전혀 기록되지 않았다.
-- 주문은 계속 pending으로 남아 마이페이지에 "결제 대기"로 노출된다.
--
-- payment_key 일치 검증(아래 22023 분기)이 이미 "Toss가 이 주문에 대해 승인한 결제"임을
-- 보장하므로, 상태가 pending/failed여도 취소를 정산하는 것이 안전하다.
-- 이용권 회수 update는 발급된 이용권이 없으면 0행으로 지나간다.
...
begin
  ...
  -- pending/failed도 허용한다: payment_key가 일치하는 이상 승인은 이미 일어났고,
  -- 취소를 기록하지 않으면 주문이 영구히 결제 대기로 남는다.
  if target_order.status not in ('paid', 'refunded', 'pending', 'failed') then
  ...
  if target_order.status <> 'refunded' then
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'settled_from_status', target_order.status
      )
    );
  end if;
