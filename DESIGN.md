...
- Evidence reviewed:
  - `src/app/page.tsx`
  - `src/components/store/CourseEnrollmentPicker.tsx`, `src/components/store/CourseEnrollmentPicker.module.css`
  ...
  - `src/components/auth/AuthForm.tsx`, `src/components/auth/AuthForm.module.css`
...
- Avoid: 학습·구매·계정 용어가 섞이는 구조, 고객 화면의 과도한 관리자 화면 느낌, 원본 사용자 ID/메타데이터 노출, 장식 위주 통계 대시보드, 팔로워 수에 따라 협찬·광고·원고료가 발생한다고 단정하거나 특정 경제적 성과를 보장하는 표현.
...
- Goals:
  ...
  - 공개 화면에서는 같은 원본 강의를 하나의 강의로 소개하고, `수강 신청` 이후에만 VOD/피드백/초밀착의 차이와 가격을 비교하게 한다.
- Non-goals:
...
- Core routes/screens:
  ...
  - `/courses/[slug]`: 강의별 판매 상세. 전체 커리큘럼과 수강 정보를 가격 없이 제공하고, `수강 신청`을 누르면 수강 방식 선택창을 연다.
  - `/my`: 마이 클래스 기본 화면.
...
- Content hierarchy for `/courses`:
  ...
  2. 이미지, 강의명, 한 줄 설명, 강의 수·재생 시간·수강 기간과 상세 CTA를 포함한 카드 그리드. 강의 가격은 목록에서 노출하지 않는다.
  3. VOD 재생, 진도 저장, 문의 방법만 간결하게 설명하는 구매 전 안내.
- Content hierarchy for course enrollment:
  1. 홈과 강의 상세는 하나의 강의 내용·커리큘럼·후기를 먼저 보여주고 가격을 노출하지 않는다.
  2. `수강 신청`을 누른 뒤 열리는 선택창에서만 `VOD 클래스`, `피드백 클래스`, `초밀착 클래스`를 비교한다.
  3. `피드백 클래스`를 추천 등급으로 강조하되, 각 카드의 CTA는 실제 DB 상품이 공개된 경우에만 결제 경로로 연결한다.
  4. 선택창은 `총 결제금액 ÷ 12`로 계산한 월 환산액을 가장 크게 보여주고, 바로 아래에 `12개월 할부 기준`과 총 결제금액을 함께 둔다. 실제 월 구독료가 아니며 무이자 적용 여부는 카드사별로 달라 Toss 결제창 확인 문구를 붙인다.
  5. `이윰 1:1 전화권`은 강의 등급 선택창에 섞지 않고 별도 상담 상품으로 노출한다. 기존 `zoom-consulting-1on1`의 과거 30분 Zoom 약정도 보존한다.
- Public course catalog card:
  ...
  - 상품명·설명·수강 기간은 대표 강의 상품, 챕터·차시·재생 시간은 공개된 강의 콘텐츠를 기준으로 한다. 가격은 수강 방식 선택창에서 각 등급 상품을 기준으로 읽는다.
- Course card:
...
- 인증 입력창은 웜 아이보리 표면, 14px 라운드, 상시 노출 라벨을 사용하고 키보드 포커스에는 갈색 테두리와 반투명 포커스 링을 함께 표시한다.
...
- New/changed components:
  ...
  - `CourseEnrollmentPicker`: `수강 신청`을 누른 뒤 포털 대화상자로 3가지 수강 등급을 비교한다. 닫기·Escape·배경 클릭과 포커스 복귀를 지원하며, DB에 공개 상품이 없으면 가격 참고값은 보이되 CTA는 `판매 준비 중`으로 비활성화한다.
  - `AccountSettings`: 카카오 로그인을 주 인증 수단으로 가정한 프로필·연결 계정·알림·탈퇴 화면. 프로필·알림·세션·회원 탈퇴를 실제 계정 상태와 연결한다.
  ...
  - `AdminProductManager`: 강의·전자책 공통 판매 단위를 검색·필터링하고 상품명, 설명, 가격, 이용 기간, 공개 상태를 등록·수정한다. VOD 등록 시 `새 강의 만들기` 또는 `기존 강의에서 챕터 선택` 중 하나를 선택해 판매 콘텐츠를 함께 연결한다. 기존 강의의 모든 챕터를 포함하려면 챕터 선택기의 `전체 선택`을 사용한다. 수정 화면에서는 향후 챕터 자동 포함이 필요한 기존 상품에 한해 전체/선택 범위를 관리할 수 있다. 상품 유형과 slug는 생성 후 잠가 주문·콘텐츠 연결을 보호하며, 상품과 실제 콘텐츠 편집은 분리한다.
  - `AdminCourseManager`: 강의 상품과 학습 콘텐츠를 연결하고 챕터·차시 등록, 수정, 순서 변경, 영상 연결과 공개 준비 상태를 관리한다. 새 VOD 상품은 `강의 연결 대기`로 먼저 보여 상품과 콘텐츠의 관계를 명확히 하며, 전체 32개 차시는 챕터 단위로 접어 운영 밀도를 유지한다. 챕터·차시의 내부 키는 시스템이 불변 값으로 자동 생성하고 관리자에게 노출하지 않는다. `1강`, `2강` 같은 번호는 현재 정렬 순서에서 계산해 중간 삽입과 순서 변경 뒤에도 자동으로 다시 매겨진다.
- Variants and states:
...
- Keyboard/focus behavior: 탭, 커리큘럼 차시, CTA, 완료 토글 모두 키보드로 접근 가능해야 한다.
  - 수강 방식 선택창은 열릴 때 닫기 버튼으로 포커스를 이동하고, Tab 포커스를 내부에 유지하며, 닫힌 뒤 실행 버튼으로 포커스를 돌려준다.
...
- Layout adaptations:
  ...
  - 수강 방식 선택창: 921px 이상 3열, 920px 이하 1열. 620px 이하에서는 화면 아래에서 올라오는 시트 형태로 표시한다.
...
- Admin curriculum ordering: 새 챕터는 목록 마지막에 추가한다. 새 차시는 추가 창에서 `맨 처음` 또는 `기존 N강 다음` 위치를 선택해 한 번에 삽입하며, 저장 후에도 위·아래 순서 버튼으로 조정할 수 있다. 버튼의 접근성 이름에는 이동 대상과 방향을 포함하며 내부 키나 데이터베이스 용어를 마이크로카피에 노출하지 않는다.
- Admin course sales scope: `전체 강의`는 이후 추가되는 챕터까지 자동 포함한다. `선택 챕터`는 체크한 챕터만 판매하고 최소 한 개 선택을 요구하며, 선택 결과를 저장하기 전 예상 공개 차시 수를 함께 보여준다.
...
- Account settings: 프로필 저장, 다른 기기 로그아웃, 알림 변경을 Supabase Auth와 동의 기록에 반영한다. 회원 탈퇴는 최근 로그인과 확인 문구를 검증한 뒤 카카오 연결을 해제하고, 거래 원장은 보존하면서 수강권·학습 기록·현재 동의 정보를 파기한 후 Auth 계정을 soft delete한다.
...
- Microcopy rules:
  ...
  - 협찬·원고료·수익 관련 숫자는 강사의 개인적인 과거 사례임을 같은 문맥에서 밝히고, 일반적인 결과나 팔로워 구간별 공식처럼 표현하지 않는다.
  - 수강생 후기는 학습 경험과 적용 과정 중심으로 노출하며, 결제 CTA 근처에는 특정 경제적 성과를 보장하지 않는다는 안내를 읽을 수 있는 크기로 제공한다.
  - 강의 목록·상세·고정 CTA에는 가격을 쓰지 않는다. `수강 신청` 이후 선택창에서만 12개월 기준 월 환산액을 1순위, 총 결제금액을 2순위로 표시한다.
  - 월 환산액은 총액을 12로 나눈 수학적 예상액이며 나누어떨어지지 않으면 `약`을 붙인다. `12개월 무이자`를 모든 카드에 보장하는 표현은 쓰지 않고 실제 할부·무이자 조건은 Toss 결제창에서 확인하게 한다.
...
- Recommended domain model:
  ...
  - `courses`: 재사용 가능한 강의 원본 메타데이터와 최초 전체 강의 상품 연결.
  - `product_course_scopes`: 강의 상품이 어떤 원본 강의를 `full | selected` 범위로 판매하는지 정의한다.
  - `product_course_scope_sections`: `selected` 범위에서 판매하는 챕터 목록. `full`은 이 테이블과 무관하게 현재 및 향후 모든 챕터를 포함한다.
  ...
  - 2026-08 멤버십 등급은 기존 `product_course_scopes`로 같은 원본 강의를 `full` 범위 공유하는 별도 `course` 상품으로 등록한다. 부가 피드백/전화 제공은 상품 혜택과 운영 안내로 이행하고, 결제 권한은 상품별 이용권으로 구분한다.
  - 등급 변경·차액 결제 정책이 확정되기 전에는 활성 멤버십 이용권이 하나라도 있으면 다른 등급 주문을 화면과 DB 양쪽에서 막고 문의로 연결한다.
  - 진도 저장 권한은 판매 상품 slug가 아니라 `product_course_scopes`가 연결한 원본 강의 slug를 기준으로 판정한다.
- Progress rule:
...
- Security:
  ...
  - 챕터 상품의 카탈로그·영상 manifest·진도 저장은 모두 같은 판매 범위 연결을 기준으로 검증한다. 화면에서 숨기는 것만으로 미구매 챕터 접근을 막았다고 간주하지 않는다.
...
- [ ] 1:1 전화권 6회의 사용 기한, 당일 취소·노쇼, 부분 사용 후 환불 기준 확정 / 상품·운영 / 상세·약관·CS 영향.
- [ ] 과제·오픈카톡·1:1 피드백의 제공 기간과 횟수, 기존 VOD 구매자의 상위 등급 업그레이드 정책 확정 / 상품·운영 / 권한·결제·CS 영향.
- [ ] 토스 상점관리자에서 12개월 무이자 대상 카드사, 행사 기간, 수수료 부담 주체를 확정 / 결제 운영 / `12개월 무이자` 확정 문구 노출 가능 여부.
